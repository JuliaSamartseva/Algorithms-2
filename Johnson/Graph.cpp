...
bool Graph::Bellman_Ford(std::vector<std::pair<int, edge>>& edges, std::vector<int>& cost, int src) {
	cost[src] = 0;
...
void Graph::Dijkstra(std::vector<std::vector<int>>& cost, int src) {
	vector<int> dist;
...
void Graph::outputCost(std::vector<std::vector<int>> cost) {
	cout << "\nCost table: \n";
	for (int i = 0; i < cost.size(); i++)
	{
...
void Graph::initialiseCostTable(std::vector<std::vector<int>>& cost) {
...
std::vector<std::pair<int, edge>> Graph::getEdges() {
...
Graph::Graph(vector<vector<int>> input, int N)
{
	this->N = N;
	this->E = input.size();
